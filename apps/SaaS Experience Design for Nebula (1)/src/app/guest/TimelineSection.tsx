// ─── TimelineSection ──────────────────────────────────────────────────────────
//
// GX-010: Infrastructure Timeline — how the observed infrastructure evolved.
//
// Philosophy: Nebula interprets history. It does not replay it.
// Each entry explains significance, not mere occurrence.
//
// Structure per entry:
//   <li>
//     <time>   Relative time ("Today", "Yesterday", "3 days ago")
//     <h3>     Headline — short, interpretive, plain language
//     <p>      Narrative — explains significance
//     <footer> Supporting metadata (observation basis, category)
//   </li>
//
// Motion: index × 0.10 delay, fade upward. No timeline drawing animations.
//
// Accessibility:
//   <section> → <ol> → <li> → <time> → <h3> → <p> → <footer>
//   Every entry includes a semantic <time datetime="..."> element.
//
// Analytics: TIMELINE_REVEALED fires once on mount.
...
// ── Relative time (GX-010) ────────────────────────────────────────────────────
// Natural language only. Falls back to "Month Year" for entries older than ~2 months.
...
function TimelineEntryCard({ entry, index, reduced }: EntryCardProps) {
  ...
  return (
    <motion.li
      ...
      "
    >
      {/* Relative time — quietly leads; datetime preserves machine-readable precision */}
      <time
        dateTime={entry.date}
      ...
      {/* Headline — interpretive, plain language */}
      <h3 className="text-[14px] font-medium text-foreground leading-snug mb-3">
        {entry.headline}
      </h3>

      {/* Narrative — explains significance, not mere occurrence */}
...
export function TimelineSection({
  ...
  useEffect(() => {
    emit("TIMELINE_REVEALED", { entryCount: timeline.length });
    ...
    >
      ...
      {hasError ? (
        ...
        <>
          ...
          {/* Divider — transitions toward Evidence section */}
          <motion.div
            initial={reduced ? false : { opacity: 0 }}
            animate={{ opacity: 1 }}
