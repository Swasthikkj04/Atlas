...
//
// Integration points as defined in GX-003.
// Each function maps directly to a backend endpoint.
// Current implementations are simulated — swap bodies for real fetch() calls.
//
// Endpoints:
//   POST /guest/understand       → postGuestUnderstand
//   GET  /guest/jobs/:id         → getGuestJob
//   GET  /guest/brief            → getExecutiveBrief
//   GET  /guest/technologies     → getTechnologies
//   GET  /guest/findings         → getObservations   (endpoint named "findings",
//                                                     product language: "observations")
//   GET  /guest/timeline         → getTimeline
//   GET  /guest/evidence         → getEvidence
...
function buildBrief(domain: string): ExecutiveBriefData {
  return {
    ...
    stats: {
      techCount:        24,
      observationCount: 3,
      evidenceCount:    7,   // matches EVIDENCE.length
      timelineCount:    5,
      criticalCount:    0,
    },
  };
...
// GX-010: Interpretive engineering timeline — not a log replay.
// Each entry explains the significance of an observed infrastructure change.
// Dates are relative to TODAY so the relative-time display stays accurate in the mock.
...
// GX-011: Full evidence entries — the appendix of the infrastructure report.
// Grouped by category. Payload is raw text, revealed on demand.
...
//
// Matches the backend contract from GX-005:
//   Request:  { domain: string }
//   Response: { jobId: string; sessionId: string; status: "QUEUED" }
//
// Simulated failure cases (swap with real fetch() in production):
//   domain starts with "net." → throws NETWORK_FAILURE  (connectivity error)
//   domain starts with "err." → throws DOMAIN_INSUFFICIENT_SIGNAL

export async function postGuestUnderstand(
  domain: string
): Promise<{ jobId: string; sessionId: string; status: string }> {
  await delay(280);
...
export async function getGuestJob(
  _jobId: string
): Promise<{ stage: number; complete: boolean }> {
  await delay(200);
...
export async function getExecutiveBrief(domain: string): Promise<ExecutiveBriefData> {
  await delay(80);
  return buildBrief(domain);
}

// ─── GET /guest/technologies ──────────────────────────────────────────────────
...
// ─── GET /guest/findings ──────────────────────────────────────────────────────
// Endpoint is "findings" per GX-003; product language (SX-006) uses "observations".
...
export async function getEvidence(): Promise<EvidenceRow[]> {
  await delay(80);
  return EVIDENCE;
}

// ─── Composite loader ─────────────────────────────────────────────────────────
// Fires all data requests in parallel during the Nebula Pause.
// Each resolves before the Brief reveal begins.

export async function loadAssessmentData(domain: string): Promise<AssessmentData> {
  const [brief, technologies, observations, timeline, evidence] = await Promise.all([
    getExecutiveBrief(domain),
    getTechnologies(),
    getObservations(),
    getTimeline(),
    getEvidence(),
