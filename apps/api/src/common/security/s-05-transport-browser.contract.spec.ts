...
describe('S-05 — Security Headers, Transport & Browser Boundary Contract Spec (API Common Security)', () => {
  describe('1. Contract Identity, Scope & Frozen Principles', () => {
    ...
    it('verifies all 4 frozen principles are defined', () => {
      expect(S05_PRINCIPLES.PERIMETER_CONTINUITY).toContain(
        'security perimeter',
      );
      expect(S05_PRINCIPLES.NO_COMPENSATING_ASSUMPTION).toContain(
        'No layer is permitted to assume',
      );
      expect(S05_PRINCIPLES.GX_WX_BROWSER_ISOLATION).toContain(
        'Same browser != same security context',
      );
      expect(S05_PRINCIPLES.DIRECT_API_INDEPENDENCE).toContain(
        'Direct API requests',
      );
    });
