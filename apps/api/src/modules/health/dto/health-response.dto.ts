...
export class DatabaseCheckDto {
  ...
  @ApiProperty({ example: 3, description: 'Database query ping latency in milliseconds.' })
  latencyMs!: number;
}

export class WorkerCheckDto {
  @ApiProperty({ example: 'UP', description: 'Worker subsystem status (UP/DOWN).' })
  status!: string;
}
...
export class MemoryCheckDto {
  ...
  @ApiProperty({ example: 180, description: 'Resident Set Size memory in megabytes.' })
  rssMB!: number;
}
...
export class HealthResponseDto {
  @ApiProperty({ example: 'HEALTHY', description: 'Overall platform health status (HEALTHY, DEGRADED, UNHEALTHY).' })
  status!: string;
  ...
  @ApiProperty({ type: SubsystemChecksDto, description: 'Comprehensive subsystem health checks.' })
