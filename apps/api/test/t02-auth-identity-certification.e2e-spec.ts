...
import { PasswordHashingService } from '../src/common/security/password-hashing';
import { UserAccountStatus, OAuthProvider } from '@prisma/client';
import { createHash, randomBytes } from 'crypto';

describe('T-02: Authentication & Identity Integrity E2E Certification', () => {
  let app: INestApplication;
  let prisma: PrismaService;

  const timestamp = Date.now();
  const userAEmail = `t02-e2e-user-a-${timestamp}@example.com`;
  const userBEmail = `t02-e2e-user-b-${timestamp}@example.com`;
  ...
  afterAll(async () => {
    ...
    if (userAId || userBId) {
      ...
      await prisma.accountReactivationToken.deleteMany({ where: { userId: { in: ids } } });
  ...
  describe('4. JWT Verification & Fail-Closed Boundaries', () => {
    ...
    it('should reject alg: none attack token with 401', async () => {
      ...
      const payload = Buffer.from(JSON.stringify({ sub: userAId, email: userAEmail, exp: Math.floor(Date.now() / 1000) + 3600 })).toString('base64url');
  ...
  describe('5. Authorization & Privilege Boundaries', () => {
    ...
    it('should prevent cross-user resource tampering', async () => {
      ...
      const checkDomain = await prisma.domain.findUnique({ where: { id: userBDomain.id } });
  ...
  describe('6. OAuth Providers Handshake', () => {
    ...
    it('should link Google OAuth account and list connected providers', async () => {
      ...
      const googleProvider = providersRes.body.providers.find((p: any) => p.provider === 'google');
  ...
  describe('7. Password Hashing & Change Security', () => {
    it('should verify PasswordHashingService enforces Argon2id', async () => {
      ...
      const isValid = await PasswordHashingService.verifyPassword(hash, initialPassword);
      expect(isValid).toBe(true);

      const isInvalid = await PasswordHashingService.verifyPassword(hash, 'wrong');
      expect(isInvalid).toBe(false);

      const posture = PasswordHashingService.evaluatePasswordStoragePosture(hash);
  ...
  describe('9. Account Lifecycle & Deactivation', () => {
    ...
    it('should request reactivation and confirm reactivation with secure token', async () => {
      ...
      const userBFromDb = await prisma.user.findUnique({ where: { id: userBId } });
