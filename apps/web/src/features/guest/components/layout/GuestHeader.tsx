...
import { LivingLogo } from "../../../../components/branding";
...
export function GuestHeader({ phase, displayDomain, onReset }: GuestHeaderProps) {
  ...
  return (
    ...
    >
      <div className="w-full px-5 sm:px-10 h-[52px] flex items-center justify-between gap-4">
        ...
        <nav aria-label="Site navigation" className="flex items-center gap-1 shrink-0">
          <a
            href="/auth/login"
            className="hidden sm:block text-[12.5px] text-muted-foreground hover:text-foreground transition-colors px-3.5 py-1.5 rounded-lg hover:bg-accent focus-ring"
          >
            Login
          </a>
          <a
            href="/auth/register"
            className="text-[12.5px] font-medium bg-foreground text-background px-3.5 py-1.5 rounded-lg hover:opacity-75 active:opacity-55 transition-opacity focus-ring"
          >
            Create Workspace
          </a>
        </nav>
