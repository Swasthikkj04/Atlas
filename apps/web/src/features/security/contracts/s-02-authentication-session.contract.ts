/**
 * S-02 — Authentication & Session Security Frontend Contract
 *
...
export const S02_INVARIANTS = {
  S02_I01: 'User access tokens must have iss="nebula-auth", aud="nebula-app", typ="user-access", and valid identity claims.',
  S02_I02: 'Admin tokens (iss="nebula-admin-auth", typ="admin-access", aal="AAL3") are strictly segregated from user tokens.',
  S02_I03: 'Refresh tokens are high-entropy, hashed at rest, single-use, and rotated upon every exchange.',
  S02_I04: 'Replay of a consumed refresh token revokes the entire token family.',
  S02_I05: 'Session state is authoritatively held on the server; client state cannot resurrect expired or revoked sessions.',
  S02_I06: 'Logout is a server-side revocation operation that invalidates refresh capabilities.',
  S02_I07: 'Global logout invalidates all sessions for the user across all devices.',
  S02_I08: 'GX cannot create, inherit, upgrade, refresh, or impersonate a User Session.',
  S02_I09: 'Only an explicit, authorized claim operation can bind a guest discovery to a user account.',
  S02_I10: 'No credentials or authentication secrets may ever be logged in plaintext.',
} as const;
...
export interface S02AttackScenario {
  ...
  readonly category: 'TOKEN_ATTACK' | 'REFRESH_ATTACK' | 'SESSION_ATTACK' | 'GX_BOUNDARY_ATTACK';
 ...
 */
export function validateAccessToken(tokenPayload: Partial<AccessTokenClaims> | null | undefined): {
  valid: boolean;
  httpStatus: 200 | 401;
  errorCode: string;
} {
...
export function evaluateRefreshTokenRotation(
  ...
  if (!incomingHash || typeof incomingHash !== 'string' || incomingHash.trim().length === 0) {
    return {
      success: false,
      httpStatus: 401,
...
export function evaluateSessionStatus(session: UserSessionRecord | null): {
  ...
  if (session.userStatus !== 'ACTIVE') {
    return { active: false, httpStatus: 401, reason: 'Account is deactivated or deleted.' };
  }

  const now = Date.now();

  if (session.revokedAt !== null) {
    return { active: false, httpStatus: 401, reason: 'Session has been revoked.' };
  }
  ...
  if (session.tokenInvalidatedAt && session.createdAt < session.tokenInvalidatedAt) {
    return { active: false, httpStatus: 401, reason: 'Session invalidated by global security action.' };
  }

  return { active: true, httpStatus: 200, reason: 'Session is active and valid.' };
}
...
export interface GxBoundaryEvaluationContext {
  ...
  readonly presentedCredentialType: 'none' | 'guest-token' | 'user-access' | 'admin-access';
...
export function evaluateGxBoundary(context: GxBoundaryEvaluationContext): {
  ...
  if (context.requestedPath.startsWith('/workspace') || context.requestedPath.startsWith('/api/v1/workspace')) {
    if (context.presentedCredentialType === 'guest-token' || context.callerIdentityType === 'GUEST') {
      return {
        allowed: false,
        effectivePlane: 'GX',
        httpStatus: 401,
    ...
    if (context.callerIdentityType === 'ANONYMOUS' || context.presentedCredentialType === 'none') {
      return {
        allowed: false,
        effectivePlane: 'GX',
        httpStatus: 401,
  ...
  if (context.requestedPath.startsWith('/guest') || context.requestedPath === '/') {
    return {
      allowed: true,
      effectivePlane: 'GX',
      httpStatus: 200,
      reason: 'Guest plane accessed; remains strictly GX regardless of user credentials.',
    };
  }

  // Explicit authorized claim action
  if (context.isClaimAction) {
    if (context.callerIdentityType !== 'USER' && context.callerIdentityType !== 'ADMIN') {
      return {
        allowed: false,
        effectivePlane: 'GX',
        httpStatus: 401,
    ...
    return {
      ...
      effectivePlane: 'WX',
      httpStatus: 200,
      reason: 'Explicit authorized claim of guest session to workspace allowed.',
    };
  }
...
export function evaluateSessionFixation(
  ...
  if (preAuthSessionId && preAuthSessionId === postAuthSessionId) {
    return { isSecure: false, reason: 'Session fixation detected: pre-auth session ID was retained.' };
  }

  return { isSecure: true, reason: 'Fresh authenticated session context established.' };
}

/**
 * 11. Credential Log Redaction Guard
 */
export function sanitizeLogData<T extends Record<string, any>>(data: T): Record<string, any> {
...
export function verifyS02Certification(candidateStatement: string): {
  ...
  const cleanCanonical = S02_CERTIFICATION_STATEMENT.toLowerCase().replace(/[^a-z0-9]/g, ' ');
