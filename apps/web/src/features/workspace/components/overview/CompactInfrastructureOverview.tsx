...
/**
 * Dynamic Curated Infrastructure Overview Card for Overview Tab (IA-1 / IA-2 / WX-909).
 *
 * Implements the domain-adaptive overview summary:
 * - Dynamic, domain-specific presentation: Displays ONLY observed infrastructure for the selected domain
 * - No clutter: Eliminates rigid empty placeholders ("Not detected") from the primary Overview tab
 * - Premium instrument feel with #FFFFFF card surface, #E1E1DC border, and subdued row hover states
 * - Direct seamless gateway: "View full infrastructure (N components) →" leads to the dedicated /workspace/infrastructure tab
 */
export const CompactInfrastructureOverview: React.FC<CompactInfrastructureOverviewProps> = ({
  ...
  const itemsToRender = resolution.detectedItems;

  return (
    <div
      className={`w-full rounded-2xl border border-[#E1E1DC] dark:border-border bg-[#FFFFFF] dark:bg-card overflow-hidden shadow-[0_1px_2px_rgba(16,24,20,0.035)] transition-all flex flex-col justify-between ${className}`}
      ...
      <div>
        ...
        <div className="px-5 py-3 border-b border-[#EEEEEB] dark:border-border-divider bg-[#FAFAF8] dark:bg-surface-secondary flex items-center justify-between">
          ...
          <span className="text-[11px] font-mono text-[#5F625F] dark:text-muted-foreground">
            {resolution.hasAnyDetected ? `${itemsToRender.length} observed` : 'Observing'}
          </span>
        </div>

        {/* 2. Dynamic Observed Rows (Curated for Selected Domain) */}
        {itemsToRender.length === 0 ? (
          <div className="px-5 py-8 text-center space-y-2">
            <p className="text-xs text-[#5F625F] dark:text-muted-foreground font-mono">
              Observing perimeter telemetry for {domainName}…
            </p>
          </div>
        ) : (
          <div className="divide-y divide-[#EEEEEB] dark:divide-border-divider">
            {itemsToRender.map((item) => {
              const IconComponent = CATEGORY_ICONS[item.id] || Cloud;
              return (
                <button
                  key={item.id}
                  type="button"
